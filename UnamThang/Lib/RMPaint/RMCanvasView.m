...
@synthesize brushScale = brushScale_;
...
{
    RMPaintStep* step = [[RMPaintStep alloc] initWithColor:self.brushColor start:start end:end size:self.brushScale];
...
- (void) setBrush:(UIImage *)image {
    ...
    size_t width = brushWidth = CGImageGetWidth(brushImage);
...
{
    brushScale_ = brushScale;
    glPointSize(brushWidth / brushScale_);
}
